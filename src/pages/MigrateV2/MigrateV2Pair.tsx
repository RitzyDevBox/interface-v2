...
export default function MigrateV2Pair({
    ...
    const liquidityToken: Token | undefined = useMemo(() => (chainId && validatedAddress ? new Token(chainId, validatedAddress, 18) : undefined), [chainId, validatedAddress]);
    ...
    const reserve1 = useMemo(() => (token1 && reserve1Raw ? CurrencyAmount.fromRawAmount(token1, reserve1Raw) : undefined), [token1, reserve1Raw]);
    ...
    if (!validatedAddress || !pair || (pair && token0AddressCallState?.valid && !token0AddressCallState?.loading && !token0AddressCallState?.error && !token0Address)) {
        ...
        <Card classes={"p-2 br-24 w-100 maw-765 mh-a"}>
            <AutoColumn gap="16px">
                ...
                {!account ? (
                    ...
                    <V2PairMigration pair={_pair} pairBalance={_pairBalance} totalSupply={_totalSupply} reserve0={_reserve0} reserve1={_reserve1} token0={_token0} token1={_token1} />
                ) : (
